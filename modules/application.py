# -*- coding: utf-8 -*-
"""Построение интерфейса в Tkinter для ThumbnailsGUI"""
import sys
...
class Application(ttk.Notebook):
    ...
    def create_menu(self):
        """
        Создает меню в приложении
        """

        pimage = Image.open(resource_path(r"modules\ico\open.png"))
        self.open_image = ImageTk.PhotoImage(pimage)
        pimage = Image.open(resource_path(r"modules\ico\save.png"))
        self.save_image = ImageTk.PhotoImage(pimage)
        pimage = Image.open(resource_path(r"modules\ico\about.png"))
        self.about_image = ImageTk.PhotoImage(pimage)
    ...
    def create_widgets(self):
        ...
        # Combobox
        self.cboConv = ttk.Combobox(self.frame1, values=list(self.convs.keys()), exportselection=0)
        ...
        # LabelInitDir
        self.lblInitDir = ttk.Label(self.frame1, text=f"Путь к {self.th.ftype} картинкам:")
        ...
        self.entInitDir.bind("<Button-3>", lambda evt, obj=self.varInitDir: self.show_menu(evt, obj))

        # ButtonInitDir
        self.btnInitDir = ttk.Button(self.frame1, text="Выбрать...", command=self.open_InitDir)
        self.btnInitDir.grid(row=3, column=1, sticky="we", **grid_params)

        # LabelOutDir
        self.lblOutDir = ttk.Label(self.frame1, text=f"Путь к {self.th.otype} картинкам:")
        ...
        self.entOutDir.bind("<Button-3>", lambda evt, obj=self.varOutDir: self.show_menu(evt, obj))

        # ButtonOutDir
        self.btnOutDir = ttk.Button(self.frame1, text="Выбрать...", command=self.open_OutDir)
        self.btnOutDir.grid(row=5, column=1, sticky="we", **grid_params)

        # ButtonConvert
        self.btnConvert = ttk.Button(self.frame1, text="Конвертировать", command=self.convert)
        ...
        self.lbFrame.grid(row=0, column=0, columnspan=3, sticky="we", **grid_params)

        # Radiobutton
        self.varDim = tk.IntVar()
        # self.varDim.set()
        self.rdb1 = ttk.Radiobutton(self.lbFrame, text="Ширина", value=1, variable=self.varDim, command=self.changeDim)
        self.rdb1.grid(row=0, column=0, sticky="w", **grid_params)
        self.rdb2 = ttk.Radiobutton(self.lbFrame, text="Высота", value=2, variable=self.varDim, command=self.changeDim)
        self.rdb2.grid(row=0, column=1, sticky="w", **grid_params)
        self.rdb3 = ttk.Radiobutton(self.lbFrame, text="Квадрат", value=3, variable=self.varDim, command=self.changeDim)
        ...
        # LabelType
        self.lblType = ttk.Label(self.frame2, text="Введите расширение картинок:")
        self.lblType.grid(row=1, column=0, sticky="we", **grid_params)

        # ComboboxType
        self.types = ["png", "jpg", "bmp"]
        self.cboType = ttk.Combobox(self.frame2, values=self.types, exportselection=0)
        ...
        # LabelSuf
        self.lblSuf = ttk.Label(self.frame2, text="Приписка (суффикс) к новым картинкам:")
        ...
        # LabelInitDir2
        self.lblInitDir2 = ttk.Label(self.frame2, text="Путь к оригинальным картинкам:")
        self.lblInitDir2.grid(row=3, column=0, columnspan=2, sticky="we", **grid_params)

        # EntryInitDir2
        self.varInitDir2 = tk.StringVar()
        self.varInitDir2.set("")
        self.entInitDir2 = ttk.Entry(self.frame2, textvariable=self.varInitDir2)
        self.entInitDir2.state(["disabled"])
        self.entInitDir2.grid(row=4, column=0, columnspan=2, sticky="we", **grid_params)
        self.entInitDir2.bind("<Button-3>", lambda evt, obj=self.varInitDir2: self.show_menu(evt, obj))

        # ButtonInitDir2
        self.btnInitDir2 = ttk.Button(self.frame2, text="Выбрать...", command=self.open_InitDir)
        self.btnInitDir2.grid(row=4, column=2, sticky="we", **grid_params)

        # LabelOutDir2
        self.lblOutDir2 = ttk.Label(self.frame2, text="Путь к новым картинкам:")
        self.lblOutDir2.grid(row=5, column=0, columnspan=2, sticky="we", **grid_params)
        ...
        self.entOutDir2.grid(row=6, column=0, columnspan=2, sticky="we", **grid_params)
        self.entOutDir2.bind("<Button-3>", lambda evt, obj=self.varOutDir2: self.show_menu(evt, obj))

        # ButtonOutDir2
        self.btnOutDir2 = ttk.Button(self.frame2, text="Выбрать...", command=self.open_OutDir)
        self.btnOutDir2.grid(row=6, column=2, sticky="we", **grid_params)

        # ButtonResize
        self.btnResize = ttk.Button(self.frame2, text="Уменьшить", command=self.resize)
        self.btnResize.grid(row=7, column=0, **grid_params)

        # ButtonEnlarge
        self.btnEnlarge = ttk.Button(self.frame2, text="Оквадратить", command=self.enlarge)
    ...
    def open_file(self, evt=None):
        ...
        :param evt:
        """
        filename = tkFD.askopenfilename(title="Выберите файл настроек", filetypes=(("Файл настроек", "BKP"),))
        if filename:
    ...
    def save_file(self, evt=None) -> None:
        """Сохранение введенных параметров
        :param evt:
        """
        filename = tkFD.asksaveasfilename(title="Сохранение настроек", filetypes=(("Файл настроек", "BKP"),))
        if filename:
            filename = os.path.splitext(filename)[0]
            db = {"conv": self.th, "resize": self.th2}
            with open(filename + ".bkp", 'wb') as f:
    ...
    def show_info(self):
        ...
        """
        msgbox.showinfo("О программе...", f"""Version: {self.version}\n© Михаил Чесноков, 2019 г.
mailto:Mihail.Chesnokov@ipsos.com""", parent=self)
    ...
    def set_types(self, evt):
        ...
        if ind == self.index(self.frame1):
            if self.convs[self.cboConv.get()] == "1":
                ...
                self.th.ftype = "jpg"
                self.th.otype = "png"
            self.lblInitDir["text"] = f"Путь к {self.th.ftype} картинкам:"
            self.lblOutDir["text"] = f"Путь к {self.th.otype} картинкам:"
            self.lblErr["text"] = ""
            self.lblErr["background"] = ""
    ...
    def enlarge(self):
        ...
        if not self.check():
            ...
            for infile in images:
                im = Image.open(infile)
                if im.size[0] > self.th2.size[0] or im.size[1] > self.th2.size[1]:
                    im.thumbnail(self.th2.size)
                if self.th2.ftype == "png":
    ...
    def createHTMDim(self, evt=None):
        ...
        if ind == self.index(self.frame1):
            ...
            images = glob.glob(self.th.outdir + "\\*" + self.th.suffix + "." + self.th.otype)
        # elif ind == self.index(self.frame2):
        else:
            outdir = self.th2.outdir
            images = glob.glob(self.th2.outdir + "\\*" + self.th2.suffix + "." + self.th2.otype)
        ...
        if len(images) > 0:
            ...
            if ind == self.index(self.frame1):
                msgbox.showerror("Ошибка!", "Картинок не найдено: " + self.th.outdir +
                                 "\\*" + self.th.suffix + "." + self.th.otype)
            elif ind == self.index(self.frame2):
                msgbox.showerror("Ошибка!", "Картинок не найдено: " + self.th2.outdir +
    ...
    def is_num(self, value) -> bool:
        """
        Проверка, что введенное значение - число. Инициализация th2 размерами в зависимости от параметров
        :param value: проверяемое значение
        :return: успешность пройденной валидации
        """
    ...
    def check(self) -> bool:
        ...
        :return: пройдена или нет проверка
        """
        ind = self.index("current")
